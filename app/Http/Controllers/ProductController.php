...
class ProductController extends Controller
{
    ...
    public function indexSamsung() {
        ...
        return view('category.samsung', compact('products'))->with('i', (request()->input('page', 1) - 1) * 5);
    }

    public function indexOppo() {
        $products = Product::where("name", "like", "%Oppo%")->paginate(15);
        return view('category.oppo', compact('products'))->with('i', (request()->input('page', 1) - 1) * 5);
    }

    public function indexIphone() {
        $products = Product::where("name", "like", "%Iphone%")->paginate(15);
        return view('category.iphone', compact('products'))->with('i', (request()->input('page', 1) - 1) * 5);
    }

    public function indexXiaomi() {
        $products = Product::where("name", "like", "%Xiaomi%")->paginate(15);
        return view('category.xiaomi', compact('products'))->with('i', (request()->input('page', 1) - 1) * 5);
    }

    public function indexVivo() {
        $products = Product::where("name", "like", "%Vivo%")->paginate(15);
        return view('category.vivo', compact('products'))->with('i', (request()->input('page', 1) - 1) * 5);
    }

    public function indexTablet() {
        $products = Product::where("name", "like", "%Tablet%")->paginate(15);
        return view('category.tablet', compact('products'))->with('i', (request()->input('page', 1) - 1) * 5);
    }

    public function indexCase() {
        $products = Product::where("name", "like", "%Case%")->paginate(15);
        return view('category.case', compact('products'))->with('i', (request()->input('page', 1) - 1) * 5);
    }

    public function indexSmartwatch() {
        $products = Product::where("name", "like", "%Smart%")->paginate(15);
        return view('category.smartwatch', compact('products'))->with('i', (request()->input('page', 1) - 1) * 5);
    }

    public function indexSoftware() {
        $products = Product::where("name", "like", "%Microsoft%")->paginate(15);
        return view('category.software', compact('products'))->with('i', (request()->input('page', 1) - 1) * 5);
    }

    public function indexHeadphone() {
        $products = Product::where("name", "like", "%Bluetooth%")->paginate(15);
        return view('category.headphone', compact('products'))->with('i', (request()->input('page', 1) - 1) * 5);
    }
}
